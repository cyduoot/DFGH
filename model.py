...
# vgg-f(cnn-f)
# vgg-f结构：http://www.vlfeat.org/matconvnet/models/imagenet-vgg-f.svg
class ImageNet(BasicModule):
    def __init__(self, bit, model_path, keep_prob, n_class):
        ...
        # conv1参数(nn.Parameter把参数加入模型的parameter列表中，实现可训练）
        # matlab文件中的权重维度为（前两个维度：滤波器尺寸(高x宽)，维度3：输入通道，维度4：输出通道）
        # 而pytorch中卷积层权重维度为输出通道 x 输入通道 x 卷积核高 x 卷积核宽
        ...
        # tensorflow中stride要求[1, stride_h, stride_w, 1]
    ...
    def forward(self, x):
        ...
        # pytorch输入图片维度：B x C x H x W，且pad方式为按维度逆序填充
